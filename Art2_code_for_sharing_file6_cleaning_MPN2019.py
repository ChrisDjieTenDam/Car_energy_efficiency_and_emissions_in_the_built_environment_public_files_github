...
It cleans the data from the 2019 Netherlands Mobility Panel.
...
P = pd.read_csv(data_directory + os.sep + 'MPN2019_originaldata/P2019.csv', usecols = ['HHID','HH_VALID','PERSID','GESLACHT','HERKOMST_w5','WERKSITUATIE_MEEST_w5','WERKURENRECENT_CONTINU','OPLEIDING','KLEEFT2'])
...
HH = pd.read_csv(data_directory + os.sep + 'MPN2019_originaldata/HH2019.csv', index_col = 0)

HH['wlokatie_bushalte1xpu'] = HH['wlokatie_bushalte1xpu']/1000 #meters to kilometers
HH['Tram_1000m'] = 0
HH.loc[(HH['wlokatie_tramhalte'] <= 1000), 'Tram_1000m'] = 1 #Because just distance to tram becomes a proxy for distance to Randstad

HH.loc[HH['HHPARK1'] == 99, 'HHPARK1'] = 0 #"no household questionnaire, no imputation from 2017 possible"

HH = HH[['HHID','JAAR','HHAUTO_N','HHPERS','N_KIND','HHPARK1','wlokatie_bushalte1xpu','Tram_1000m']]


'''XXXXX Preprocessing the Car data questionnaires  and adding (privacy_sesitive) plates XXXXX ''' #

Car = pd.read_csv(data_directory + os.sep + 'MPN2019_originaldata/Car2019.csv', index_col = 0)#HH_Valid from this dataset does NOT overlap with the HH_Valid from HH and P
...
#AUTO_1KM 2017 matches categories in 2018 and 2019 (same limits)
for km in ['AUTO1_KM','AUTO2_KM','AUTO3_KM','AUTO4_KM','AUTO5_KM']: 
...
Plate = pd.read_csv(data_directory + os.sep + 'MPN2019_originaldata/Plate_private2019.csv', index_col = 0)
...
for jaar in ['KAUTO1_BOUWJAAR','KAUTO2_BOUWJAAR','KAUTO3_BOUWJAAR','KAUTO4_BOUWJAAR','KAUTO5_BOUWJAAR']: #because cars built before 2000 are categorized in 2020 (>20 years old)
    ...
    Car.loc[Car[jaar] == 2, jaar] = 1985 #1990-2000: 35 years old

Car['age1'] = 2019 - Car['KAUTO1_BOUWJAAR']
Car['age2'] = 2019 - Car['KAUTO2_BOUWJAAR']
Car['age3'] = 2019 - Car['KAUTO3_BOUWJAAR']
Car['age4'] = 2019 - Car['KAUTO4_BOUWJAAR']
Car['age5'] = 2019 - Car['KAUTO5_BOUWJAAR']
...
#You need one entry for every car of multicar households
...
Car.loc[Car['Car_year_MPN'] ==  0, 'Car_year_MPN'] = np.nan #Car was not built when Jesus was born
...
Weights = pd.read_csv(data_directory + os.sep + 'MPN2019_originaldata/Weights2019.csv', index_col = 0)
...
MPN2019 = pd.merge(P, HH, left_on = 'HHID', right_on = 'HHID', how = 'left')
MPN2019 = pd.merge(MPN2019, Weights, left_on = 'HHID', right_on = 'HHID', how = 'left') 
MPN2019 = pd.merge(MPN2019, Car, left_on = 'HHID', right_on = 'HHID', how = 'left') #One entry per household car
MPN2019.loc[MPN2019['HH_cars'].isna(), 'HH_cars'] = MPN2019['HHAUTO_N'] #HHAUTO_N is car-count from household-questionnaire 
MPN2019 = MPN2019[(MPN2019['HH_VALID']==1)|(MPN2019['HH_VALID']==2)] ##All HH members must have filled in the personal questionnaire. Otherwise, sociodemographic variables and weight factors become less reliable

MPN2019.loc[MPN2019['AUTO'].isna(), 'AUTO'] = 0 #So carless households will also have a CARID
MPN2019['CARID'] = MPN2019['HHID']*10 + MPN2019['AUTO']
MPN2019 = MPN2019.drop_duplicates(subset = 'CARID') 

MPN2019.loc[(MPN2019['HH_cars'] > 2), 'HH_cars'] = 2

#Assuming HHPERS includes the children under twelve 
MPN2019['HH_adults'] = MPN2019['HH_18to39'] + MPN2019['HH_40to59'] + MPN2019['HH_60plus'] #Ensures that missing household members are not included in the count

#Converting non-age related sociodemographics to fractions to avoid multicollinearity
MPN2019['FracAdultMales'] = MPN2019['HH_AdultMales']/MPN2019['HH_adults']
MPN2019['FracAdultWorkers'] = MPN2019['HH_AdultWorkers']/MPN2019['HH_adults']
MPN2019['FracAdultHighedu'] =  MPN2019['HH_highereducated']/MPN2019['HH_adults']

'''XXXXX Adding the incomes and PC6s (privacy-sensitive mergers) XXXX'''

#Adding the PC6 of each household 
PC6 = pd.read_csv(data_directory + os.sep + 'MPN2019_originaldata/HH_PC4_private2019.csv', index_col = 0)
PC6 = PC6[['HHID','WOONPC6']]
MPN2019combi = pd.merge(MPN2019, PC6, left_on = 'HHID', right_on = 'HHID', how = 'left')

#Adding data on each PC6 (including distances to allhuge, alllarge, and allmedium centers from Snellius) #Data seems to be missing for some households (Postcodes too small?)
PC6_vars = pd.read_csv('Datasets/PC6_2018_bewerkt_plusallmediumlargerhugecenters.csv', index_col = 0)
MPN2019combi['WOONPC6'] = MPN2019combi['WOONPC6'].str.upper() #Cause some postcodes are in small letters
MPN2019combi = pd.merge(MPN2019combi, PC6_vars, left_on = 'WOONPC6', right_on = 'PC6', how = 'left')

#Adding the household income
IncomePrivate = pd.read_csv(data_directory + os.sep + 'MPN2019_originaldata/Income_private2019.csv', index_col = 0)
IncomePrivate = IncomePrivate[['PERSID','HHBRUTOINK1_w5']]#See original .sav file for metadata
MPN2019combi = pd.merge(MPN2019combi, IncomePrivate, left_on = 'PERSID', right_on = 'PERSID', how = 'left')
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 29, 'HHBRUTOINK1_w5'] = 28 #Unknown income category
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'].isna(), 'HHBRUTOINK1_w5'] = 28 #Unknown income category

#Income decategorization because unequal income-bins (that also vary per MPN-wave)
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 28, 'HHBRUTOINK1_w5'] = 14 #Unknown to average
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 1, 'HH_inc_real'] = (0 + 5100)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 2, 'HH_inc_real'] = (5100 + 7000)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 3, 'HH_inc_real'] = (7000 + 9000)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 4, 'HH_inc_real'] = (9000 + 10200)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 5, 'HH_inc_real'] = (10200 + 12200)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 6, 'HH_inc_real'] = (12200 + 14100)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 7, 'HH_inc_real'] = (14100 + 16000)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 8, 'HH_inc_real'] = (16000 + 17300)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 9, 'HH_inc_real'] = (17300 + 19200)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 10, 'HH_inc_real'] = (19200 + 22400)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 11, 'HH_inc_real'] = (22400 + 26300)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 12, 'HH_inc_real'] = (26300 + 29500)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 13, 'HH_inc_real'] = (29500 + 36500)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 14, 'HH_inc_real'] = (36500 + 43500)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 15, 'HH_inc_real'] = (43500 + 57600)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 16, 'HH_inc_real'] = (57600 + 73000)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 17, 'HH_inc_real'] = (73000 + 87100)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 18, 'HH_inc_real'] = (87100 + 116500)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 19, 'HH_inc_real'] = (116500 + 145400)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 20, 'HH_inc_real'] = (145400 + 174200)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 21, 'HH_inc_real'] = (174200 + 203600)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 22, 'HH_inc_real'] = (203600 + 231800)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 23, 'HH_inc_real'] = (231800 + 261300)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 24, 'HH_inc_real'] = (261300 + 290700)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 25, 'HH_inc_real'] = (290700 + 319500)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 26, 'HH_inc_real'] = (319500 + 349000)/2
MPN2019combi.loc[MPN2019combi['HHBRUTOINK1_w5'] == 27, 'HH_inc_real'] = 349000 + (349000-3195000)/2 #Highest income plus half the difference with last bin? Reconsider later.

#Dropping useless columns, renaming columns, and then dropping all columns not to be used immediately (for privacy-protection purposes)
MPN2019combi = MPN2019combi[['HHID','CARID','JAAR','HH_VALID','HH_weight','FracAdultHighedu','FracAdultWorkers','N_KIND','HH_12to17','HH_adults','HH_18to39','HH_40to59','HH_60plus','HH_Dutch','FracAdultMales','HHBRUTOINK1_w5','HH_inc_real','OAD_PC5','AFS_SUPERM','AFS_OPRIT','AFS_TREINS','AFS_TRNOVS','km_center','km_mediumcenter','km_largercenter','km_hugecenter','ndvi_avg','landuse_idx5','f_4plus','wlokatie_bushalte1xpu','Tram_1000m','HHPARK1','HH_cars', 'Prob_carisusedmost','Car_ownership_MPN', 'Car_design_MPN','Car_year_MPN','Fueltype_MPN','Car_kg_RDW','Nettmaxpower_RDW','Fuel_RDW_combined', 'CO2_RDW_combined','vkms_elec_TNO','MJ/vkm_EVs_TNO','MJ/vkm_fuel_PHEV_TNO','Fuel_norm_accordingtotravelcard','Fuel_real_travelcard','CO2_norm_accordingtotravelcard','CO2_real_travelcard']]
MPN2019combi.columns = ['HHID','CARID','JAAR','HH_VALID','HH_weight','FracAdultHighedu','FracAdultWorkers','HH_under12','HH_12to17','HH_adults','HH_18to39','HH_40to59','HH_60plus','HH_Dutch','FracAdultMales','HH_inc','HH_inc_real','Density_PC5','km_super','km_highway','km_station','km_bigstation','km_center','km_mediumcenter','km_largercenter','km_hugecenter','NDVI','Landuse','f_4plus','km_bus','Tram_1000m','Parkingspot','HH_cars','Prob_carisusedmost', 'Car_ownership_MPN', 'Car_design_MPN','Car_year_MPN','Fueltype_MPN','Car_kg_RDW','Nettmaxpower_RDW','Fuel_RDW_combined', 'CO2_RDW_combined','vkms_elec_TNO','MJ/vkm_EVs_TNO','MJ/vkm_fuel_PHEV_TNO','Fuel_norm_accordingtotravelcard','Fuel_real_travelcard','CO2_norm_accordingtotravelcard','CO2_real_travelcard']

#Final selection based on privacy-sensitive data and saving
MPN2019combi = MPN2019combi[MPN2019combi['Density_PC5'] >= 0] 
MPN2019combi.loc[MPN2019combi['km_bus'].isna(), 'km_bus'] = MPN2019combi['km_bus'].mean() #The KiM seems to have not fixed the CBS subscript postcodes
MPN2019combi.to_csv(data_directory + os.sep + 'MPN_processed_data/MPN2019_combinedfile_1entrypercar.csv')


'''XXXXX Getting the descriptive statistics XXXXX '''
# # MPN2019independent = MPN2019combi[MPN2019combi['HH_cars'] > 0]
# MPN2019independent = MPN2019combi[['GESLACHT','OPLEIDING','KLEEFT2','WERKSITUATIE_MEEST_w5','Density_PC5','km_station','km_center','NDVI','Landuse','km_bus','ParkingSpot','HH_inc','HH_undertwelve','HH_workers','HH_students','HH_other','HH_highedu']]
# MPN2019independent.columns = ['GESLACHT','OPLEIDING','KLEEFT2','WERKSITUATIE_MEEST_w5','Address density PC5 (addresses/km2)','Distance to train station (km)','Distance to center (km)','Green space (NDVI)','Landuse mix entropy (index)','Distance to bus stop (km)','Parking','Income (ordinal)','Young children (# people)','Workers (# people)','Students (# people)','Other (# people)','Higher educated (# people)']
# MPN2019description = MPN2019independent.describe()
# MPN2019description = MPN2019description.round(2)
# MPN2019description = MPN2019description.T
# MPN2019description = MPN2019description[['min','25%','50%','mean','75%','max','std']]
# print(MPN2019description.to_latex())
